...
```
will compile the file `SimpleGinRummyPlayer.java`, along side any other necessary files. The agent specified by the `SimpleGinRummyPlayer` class should be derived from `eaai.ginrummy.game.GinRummyPlayer` in order to work properly with this tournament software. Classes can exist within any package structure, however the following assumes that all classes reside in the default package.
...
usage: java -jar vermouth.jar
 ...
 -rr,--roundrobin        run a round robin competition between the
                         competitors
 -n,--games <games>      number of rounds to evaluate per match
...
- `agents.csv`: Stores the agent id (by index) and the associated agent class URL. The data is formatted as a CSV which is ammenable to loading and data processing in common languages (python/R). This is mostly for review or collating results.
...
- `stats.csv`: __TODO__ Stores runtime statistics of different agent calls.
